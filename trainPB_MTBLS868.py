...
if __name__ == "__main__":
    ...
    expParams = {"MTBLS868": {"polarities": {"positive": "Q Exactive (MS lvl: 1, pol: +)"},
                              "minRT":100, "maxRT":2200, "RTpeakWidth":[2,30], 
                              "intraScanMaxAdjacentSignalDifferencePPM":15, "interScanMaxSimilarSignalDifferencePPM":3,
                              "noiseLevel":1E3, "minIntensity":3E3},}
    ...
    inFiles = {
        "HD871_2_1" : {"file": "./Data/MTBLS868/HD871_2_1.mzXML", "params": "MTBLS868"},
        "HD871_2_4" : {"file": "./Data/MTBLS868/HD871_2_4.mzXML", "params": "MTBLS868"},
    }
    exFiles = {
        "HD871_2_2" : {"file": "./Data/MTBLS868/HD871_2_2.mzXML", "params": "MTBLS868"},
        "HD871_2_3" : {"file": "./Data/MTBLS868/HD871_2_3.mzXML", "params": "MTBLS868"},
    }
    ...
    peakBotModelFile = "./temp/PBmodel_MTBLS868.model.h5"
    ...
    if args.trainModels:
        histAll = None
        try:
            os.remove(os.path.join(".", "Data", "History_MTBLS868.pandas.pickle"))
        except Exception:
            pass
        ...
        for i in range(args.replicates):
            ...
            def rotate(l, n):
                n = n % len(l)
                return l[n:] + l[:n]
            headers, peaks       = peakbot.readTSVFile("./Reference/MTBLS868_Peaks.tsv"      , convertToMinIfPossible = True)
            headers, walls       = peakbot.readTSVFile("./Reference/MTBLS868_Walls.tsv"      , convertToMinIfPossible = True)
            headers, backgrounds = peakbot.readTSVFile("./Reference/MTBLS868_Backgrounds.tsv", convertToMinIfPossible = True)
            print("rotating training data by", int(i * len(peaks)/5), "of", len(peaks))
            peaks = rotate(peaks, int(i * len(peaks)/5))
            a = int(len(peaks)*0.62)
            peaksTrain = peaks[:a]
            peaksVal   = peaks[a:]
            ...
            with strategy.scope():
                ...
                ## (no changes are required here)
                histAll.to_pickle(os.path.join(".", "Data", "History_MTBLS868.pandas.pickle"))
    ...
    p9.ggsave(plot=plot, filename=os.path.join(".", "Data", "SummaryPlot_MTBLS868.png"), width=5.2, height=4, dpi=300)
